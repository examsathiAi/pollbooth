# PollBooth — Terms and Conditions

_Last updated: [DATE OF LAUNCH]_

## 1. About PollBooth

PollBooth ("we", "us", "the Platform") is a public opinion polling platform operated in India.
Users can vote on polls, share opinions, suggest polls, report civic issues, and (where
enabled) create their own polls subject to review. These Terms govern your use of the
PollBooth website and mobile applications.

By creating an account or otherwise using PollBooth, you agree to these Terms, our Privacy
Policy, and our Community Guidelines.

## 2. Eligibility

You must be at least 16 years old to create a PollBooth account. By signing up, you confirm
you meet this minimum age requirement. If we learn that an account belongs to someone
below this age, we reserve the right to suspend or terminate it.

## 3. Your Account

- You are responsible for the phone number used to register and for all activity under
  your account.
- One account per person. Creating multiple accounts to manipulate votes, opinions, or
  poll results is prohibited and may result in suspension of all associated accounts.
- You may request deletion of your account at any time (see Section 9, Grievance
  Redressal, for how).

## 4. Voting, Opinions, and Content Rules

- One vote per user per poll. Attempts to circumvent this (multiple accounts, automated
  tools, etc.) are prohibited.
- Opinions must be your own genuine view, kept to the platform's character limit, and
  must not contain: hate speech, harassment, threats, sexually explicit content, content
  that promotes self-harm, misinformation intended to deceive, or content violating any
  applicable law.
- We do not permit content that is defamatory, discloses another person's private
  information without consent, or infringes anyone's intellectual property.
- Violations are handled under our escalating moderation policy: warning, temporary
  restriction on posting opinions, and, for repeat or severe violations, permanent
  restriction. Your ability to vote may be preserved separately from your ability to
  post opinions, depending on the nature of the violation.
- We may remove content that violates these Terms without prior notice.

## 5. Political and Election-Related Polls

- Polls in the Politics/Civic category may be automatically restricted during periods
  identified as active election blackout windows for a given region, in line with
  applicable election law. This is enforced by the Platform and is not adjustable by any
  user, including poll creators.
- PollBooth does not host or permit exit polls (asking how someone already voted in a real
  election) during any legally restricted period.

## 6. User-Created Polls (Paid or Credit-Based Poll Creation)

Where this feature is enabled for your account:
- You may create your own poll by paying a fee or using credits earned on the Platform.
- Every user-created poll is subject to review and approval by PollBooth before it goes live.
  We may reject any poll for any reason, including but not limited to violation of these
  Terms, applicable law, or our editorial standards, without obligation to refund credits
  already spent on rejected content, except where required by law.
- If approved, your poll will display your username as the creator, and will run
  according to the duration, regional targeting, and reach limits you selected (or the
  Platform's defaults) at the time of creation.
- **You are solely responsible for the content of any poll you create**, including its
  question and answer options. You agree that:
  - You will not create a poll that is defamatory, discriminatory, hateful, obscene,
    or that infringes any third party's rights.
  - You will not use a user-created poll to harass, target, or single out a private
    individual.
  - You bear full legal and civil responsibility for any consequences arising from a
    poll you create, including but not limited to claims of defamation, harassment, or
    violation of any person's rights. PollBooth's review and approval of a poll does not
    transfer this responsibility to PollBooth or constitute an endorsement of its content.
  - PollBooth may remove your poll at any time, including after approval, if it is later
    found to violate these Terms or applicable law, and may suspend your ability to
    create further polls.
- Payment for poll creation (where made in cash rather than credits) is subject to our
  refund policy: [TO BE DEFINED - e.g., non-refundable once a poll is approved and live;
  refundable if rejected before going live].

## 7. Sponsored and Commercial Content

Polls or content marked "Sponsored" are paid placements and are always clearly labeled as
such. Sponsored content never influences or is blended into organically-ranked results,
trending calculations, or "Top Voted" listings.

## 8. Data Use and Privacy

Our collection, use, and retention of your data is governed by our Privacy Policy. In
summary:
- We collect your phone number (hashed for lookup), demographic information you choose
  to provide (age bracket, gender, location, etc.), and your voting/opinion activity.
- We may share **aggregated, anonymized** data (never data that identifies you
  individually) with research or analytics partners, only where you have separately
  consented to this specific use.
- Server and access logs are retained for a maximum of 180 days, after which they are
  automatically deleted, except where longer retention is required by law or for
  active investigation of abuse.

## 9. Grievance Redressal

If you have a complaint about content on PollBooth, believe your rights have been violated,
or wish to request data deletion or correction, you may contact our Grievance Officer:

**Grievance Officer**: [NAME]
**Email**: [GRIEVANCE EMAIL ADDRESS]
**Response time**: We will acknowledge your complaint within 24 hours and aim to resolve
it within 15 days, in accordance with applicable Indian law (Information Technology
Rules, 2021 and the Digital Personal Data Protection Act, 2023).

## 10. Limitation of Liability

PollBooth provides the Platform on an "as is" basis. We are not liable for content posted by
users (including user-created polls, opinions, or civic reports), except as required by
applicable law. Our liability for any claim arising from your use of the Platform is
limited to the maximum extent permitted by Indian law.

## 11. Changes to These Terms

We may update these Terms from time to time. Continued use of PollBooth after changes take
effect constitutes acceptance of the updated Terms. Material changes will be notified to
users through the Platform.

## 12. Governing Law

These Terms are governed by the laws of India. Any disputes will be subject to the
exclusive jurisdiction of the courts of [CITY], India.

---

_This document is a starting draft based on PollBooth's actual features as designed. Sections
marked with [BRACKETS] require a specific decision or a lawyer's input before publishing
live. This is not a substitute for review by a qualified Indian technology/data-privacy
lawyer before real users' data is collected under these terms._
