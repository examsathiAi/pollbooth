  ...
  return (
    <main className="min-h-screen bg-slate-950 p-6 text-slate-100">
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-400">Category hub</p>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6">
          ...
          <div className="mt-4 space-y-3">
            {trending.map((poll) => (
              <div key={poll.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                <p className="font-medium text-white">{poll.question}</p>
                <p className="mt-1 text-sm text-slate-400">{poll.total_votes} votes</p>
              </div>
            ))}
          </div>
        </section>
