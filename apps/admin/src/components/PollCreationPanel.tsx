...
export function PollCreationPanel() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Poll created and sent for review.");
      setQuestion("");
      setOptions(["Yes", "No"]);
      setCategory("CIVIC");
      setRegion("ALL");
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-black/20">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">This submits directly to the real poll creation endpoint and supports commercial or organic labeling.</p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-sm text-slate-300">
          <div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-cyan-400" /> Live endpoint</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-300">Target region</span>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-3 text-sm text-slate-200">
            <p className="font-medium text-slate-300">Review flow</p>
            <p className="mt-1 text-sm text-slate-400">New polls are queued for review and only become public after approval.</p>
          </div>

          <label className="flex items-center gap-3 rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-3 text-sm text-slate-200">
            <input type="checkbox" checked={isCommercial} onChange={(e) => setIsCommercial(e.target.checked)} className="h-4 w-4 rounded border-slate-600 bg-slate-900" />
            Mark this poll as sponsored/commercial
          </label>
        </div>
          ...
          <div className="space-y-2">
            {options.map((option, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                {options.length > 2 && (
                  <button type="button" onClick={() => removeOption(index)} className="rounded-xl border border-slate-700 px-3 py-2 text-sm text-slate-300">Remove</button>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={isSubmitting || !question.trim() || options.filter(Boolean).length < 2} className="inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-violet-500 disabled:opacity-60">
            {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />} Create poll
          </button>
