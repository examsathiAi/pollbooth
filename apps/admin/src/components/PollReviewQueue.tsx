  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-black/20">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-medium uppercase tracking-[0.25em] text-violet-400">Pending review</p>
          <h2 className="text-xl font-semibold text-white">Draft polls awaiting approval</h2>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-950/80 px-3 py-2 text-sm text-slate-300">
          <div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-cyan-400" /> Needs review</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-400">No polls currently awaiting review.</div>
      ) : (
        <div className="space-y-3">
          {polls.map((poll) => (
            <article key={poll.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm text-slate-400">{poll.category}</p>
                  <h3 className="mt-1 font-semibold text-white">{poll.question}</h3>
                  <p className="mt-2 text-sm text-slate-400">Created {new Date(poll.created_at).toLocaleDateString()}</p>
                </div>
